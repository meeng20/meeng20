...
if (isset($_GET['id']) && isset($_GET['email']) && isset($_GET['password'])) {
	...
	$sql = "SELECT * FROM clientsrecord WHERE id='$id' AND email='$email' AND password='$password'";
    ...
    if($numrows == 0){
        ...
        header("location: messagewalkin.php?msg=activation_failure");
    	exit();
    } else if($numrows == 1) {
		// Great everything went fine with activation!
        header("location: messagewalkin.php?msg=activation_success");
    	exit();
    }
} else {
	// Log this issue of missing initial $_GET variables
	header("location: messagewalkin.php?msg=missing_GET_variables");
    exit(); 
}
?>